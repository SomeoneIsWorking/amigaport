...
| Native interception | Image-qualified registry, scoped current-key suppression, host-owned subroutine frames, and bounded guest-call continuations | `src/override_registry.*`, `src/executor.cpp` | `register_override`, `call`, `continue_call`, `call_interrupt` | `docs/project-state.md` |
