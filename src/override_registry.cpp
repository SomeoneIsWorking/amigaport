...
void OverrideRegistry::remove(ExecutionIdentity identity) {
    overrides_.erase(identity);
}
...
NativeOverride *OverrideRegistry::find(ExecutionIdentity identity) {
    ...
    auto iterator = overrides_.find(identity);
...
OverrideRegistry::ScopedSuppression::~ScopedSuppression() {
    registry_.suppressions_.pop_back();
}
